...
const App = () => {
  ...
  const onSubmit = useCallback(
    ...
    [data, form.name, form.username] // useCallback 은 첫번째 인자는 배열 두번쨰 인자는 해당 인자의 값 바뀔 때  함수 생성
  );
  // 항목을 삭제하는 함수
  const onRemove = useCallback(
    id => {
      setData({
        ...
        // 첫 번째 인자로 콜백 함수(콜백 함수의 첫 번째 인자는 순회하는 배열의 값(value), 두 번째 인자는 그 값의 인덱스(index), 세 번째 인자는 배열 자체(array)이다)
      });
    },
